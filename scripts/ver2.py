...
def parse_object_number(raw) -> dict | None:
    ...
    # g3 so lassen (führende Nullen wichtig)
...
def copy_matches(files, out_dir: Path, seen_outputs: set, meta_row: dict, copied_rows: list):
...
def main():
    ...
    # 2) AEG: nur Excel-Zeilen 2000–4500
    # Annahme: Excel-Zeilen zählen ab 1 inkl. Kopfzeile,
    # pandas iloc ist 0-basiert und end-exklusiv -> 2000..4500 ≈ iloc[1999:4500]
    process_list(
        path_excel=AEG_EXCEL,
        out_dir=OUT_AEG,
        log_prefix="aeg_3000_4500",
        row_slice=slice(3500, 4500),  # inkl. 3501, exkl. 3501
        header_name=T1_HEADER
    )
